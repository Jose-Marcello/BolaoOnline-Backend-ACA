...
namespace ApostasApp.Core.Application.Services.Rodadas
{
  public class RodadaService : BaseService, IRodadaService
  {
    ...
    public async Task<ApiResponse<IEnumerable<JogoDto>>> ObterJogosPorRodada(Guid rodadaId)
    {
      var response = new ApiResponse<IEnumerable<JogoDto>>();
      try
      {
        ...
        {
          response.Message = "Nenhum jogo encontrado para esta rodada.";
          response.Success = true; // Sucesso técnico, mas lista vazia
          response.Data = Enumerable.Empty<JogoDto>();
          return response;
        }

        // Mapeamento manual ou via AutoMapper para JogoDto
      ...
      catch (Exception ex)
      {
        ...
        response.Message = $"Erro ao processar jogos: {ex.Message}";
      }
      return response;
    }
