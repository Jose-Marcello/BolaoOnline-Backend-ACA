  ...
  public class CampeonatoService : BaseService, ICampeonatoService
  {
    ...
    public async Task<ApiResponse<ApostasCampeonatoTotaisDto>> ObterTotaisDashboard(Guid campeonatoId)
    {
      ...
      try
      {
        ...
        // 1. SETOR VINCULADOS (Adesões Integrais)
        var adesoes = await _apostadorCampeonatoRepository.ObterAdesoesPorCampeonatoIdAsync(campeonatoId);
        totaisDto.QuantVinculados = adesoes?.Count() ?? 0;
        totaisDto.ArrecadacaoVinculados = totaisDto.QuantVinculados * (campeonato.CustoAdesao ?? 0);
        totaisDto.PremioFinalCampeonato = totaisDto.ArrecadacaoVinculados * 0.80m;

        // 2. SETOR RODADA CORRENTE (EM DISPUTA)
        var rodadaCorrente = (await _rodadaRepository.ObterRodadaCorrentePorCampeonato(campeonatoId));
        if (rodadaCorrente != null)
        {
          totaisDto.RodadaCorrenteId = rodadaCorrente.NumeroRodada.ToString();
          var t = await _apostaRodadaRepository.ObterTotaisApostasAvulsas(rodadaCorrente.Id);
          totaisDto.QuantApostasCorrentes = t.NumeroDeApostas;
          totaisDto.ArrecadacaoCorrente = t.ValorTotal;
          totaisDto.PremioCorrente = t.ValorTotal * 0.70m;
        }

        // 3. SETOR EM APOSTA (PRÓXIMA A FECHAR)
        var rodadaEmAposta = (await _rodadaRepository.ObterRodadasEmApostaPorCampeonato(campeonatoId))
                             .OrderBy(r => r.DataFim).FirstOrDefault();
        if (rodadaEmAposta != null)
        {
          totaisDto.RodadasEmApostaIds = rodadaEmAposta.NumeroRodada.ToString();
          var t = await _apostaRodadaRepository.ObterTotaisApostasAvulsas(rodadaEmAposta.Id);
          totaisDto.QuantApostasAvulsas = t.NumeroDeApostas; // Correção aqui
          totaisDto.ArrecadacaoAvulsaRodada = t.ValorTotal;
          totaisDto.PremioLiquidoRodada = t.ValorTotal * 0.70m;
        }
