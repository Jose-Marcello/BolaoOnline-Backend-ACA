...
namespace ApostasApp.Core.Web.Controllers // Namespace CORRIGIDO para ApostasApp.Core.Web.Controllers
{
  ...
  public class CampeonatoController : BaseController
  {
    ...
    [AllowAnonymous]
    [HttpGet("{id}/totais")]
    public async Task<ActionResult<ApiResponse<ApostasCampeonatoTotaisDto>>> ObterTotais(Guid id)
    {
      var response = await _campeonatoService.ObterTotaisDashboard(id);
