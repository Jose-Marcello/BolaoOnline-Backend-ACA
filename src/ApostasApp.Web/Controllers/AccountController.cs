...
using ApostasApp.Core.Domain.Interfaces.Usuarios; // Para IUsuarioService
using ApostasApp.Core.Domain.Models.Notificacoes; // Para NotificationDto (se usar diretamente)
...
using System; // Para Guid, se necessário em algum método
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> ConfirmEmail([FromQuery] string userId, [FromQuery] string code)
        {
            ...
            if (string.IsNullOrEmpty(userId) || string.IsNullOrEmpty(code))
            {
                ...
                Notificador.Handle(new Notificacao("PARAMETROS_INVALIDOS", "Erro", "Parâmetros de confirmação de e-mail inválidos."));
        ...
        public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordRequestDto request)
        {
            ...
            if (string.IsNullOrEmpty(userId))
            {
                // Cria uma ApiResponse de falha para retornar de forma consistente
                var apiResponse = new ApiResponse<bool>(false, false);
                Notificador.Handle(new Notificacao("NAO_AUTENTICADO", "Erro", "Usuário não autenticado ou ID do usuário não encontrado no token."));
                apiResponse.Notifications = ObterNotificacoesParaResposta().ToList();
                return CustomApiResponse(apiResponse);
            }
