...
namespace ApostasApp.Core.Infrastructure.Data.Repository.Apostas
{
  public class PalpiteRepository : Repository<Palpite>, IPalpiteRepository
  {
    ...
    public async Task<Palpite> ObterPalpiteDaAposta(Guid apostaId, Guid jogoId)
    {

      // Forçamos a comparação de valores
      return await Db.Palpites
          .Where(p => p.ApostaRodadaId.ToString().ToLower() == apostaId.ToString().ToLower())
          .Where(p => p.JogoId.ToString().ToLower() == jogoId.ToString().ToLower())
          .FirstOrDefaultAsync();
